...
export default function ProductDetailPage({
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#F8F5F1] pt-20 sm:pt-28 pb-28 lg:pb-32 text-[#171615]">
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-5 space-y-6 sm:space-y-8">
            ...
            {/* Desktop Quantity & Add To Cart CTA */}
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="flex items-center border border-[#C98F78]/30 rounded-full bg-[#F8F5F1] px-2 sm:px-3 py-1.5 shrink-0">
                  <button
                    ...
                    className="min-w-[32px] h-8 flex items-center justify-center font-mono text-sm text-[#171615]/70 hover:text-[#171615] active:scale-95 transition-transform"
                ...
                <button className="flex-1 bg-[#171615] text-[#F8F5F1] py-3.5 sm:py-4 px-4 sm:px-8 rounded-full font-sans text-[11px] sm:text-xs uppercase tracking-[0.2em] font-semibold hover:bg-[#C98F78] active:scale-[0.99] transition-all duration-300 shadow-md truncate">
                  Add to Ritual — {totalPrice}
                </button>
              </div>

              <div className="flex items-center justify-center gap-4 sm:gap-6 font-sans text-[10px] uppercase tracking-[0.12em] sm:tracking-[0.15em] text-[#171615]/60 pt-1">
                <span className="flex items-center gap-1.5">
                  <ShieldCheck className="w-3.5 h-3.5 text-[#C98F78] shrink-0" /> Dermatologist Tested
                </span>
                <span className="flex items-center gap-1.5">
                  <Droplets className="w-3.5 h-3.5 text-[#C98F78] shrink-0" /> Fragrance Free
                </span>
              </div>
            </div>
        ...
        <div className="mt-14 sm:mt-20 pt-10 sm:pt-16 border-t border-[#C98F78]/20 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="flex items-center gap-4 sm:gap-6 border-b border-[#C98F78]/20 pb-3 overflow-x-auto no-scrollbar">
              <button
                onClick={() => setActiveTab("morning")}
                className={`flex items-center gap-2 font-sans text-xs uppercase tracking-[0.18em] font-semibold pb-2 border-b-2 transition-all cursor-pointer whitespace-nowrap shrink-0 ${
                  activeTab === "morning"
              ...
              {activeTab === "morning" && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.2em] text-[#C98F78]">
                    <Clock className="w-3.5 h-3.5 shrink-0" /> Frequency: {product.routine.frequency}
                  </div>
              ...
              {activeTab === "results" && (
                <div className="w-full grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                  {product.clinicalResults.map((res, i) => (
                    <div key={i} className="bg-[#F8F5F1] p-4 rounded-xl sm:rounded-2xl border border-[#C98F78]/20 space-y-1">
                      <div className="font-serif text-2xl sm:text-3xl text-[#C98F78] font-semibold">{res.metric}</div>
          ...
          <div className="lg:col-span-5 space-y-3">
            ...
            <div className="border border-[#C98F78]/25 rounded-2xl bg-[#F8F5F1] overflow-hidden">
              <button
                ...
                className="w-full p-4 sm:p-5 flex items-center justify-between text-left font-serif text-base sm:text-lg text-[#171615] cursor-pointer hover:bg-[#E9D8D0]/20 transition-colors"
              >
                <span>Full Ingredients List (INCI)</span>
                <ChevronDown
                  className={`w-4 h-4 text-[#C98F78] shrink-0 transition-transform duration-300 ${
              ...
              {openInci && (
                <div className="p-4 sm:p-5 border-t border-[#C98F78]/15 bg-[#F4EFEA]/40 font-mono text-[10px] sm:text-[11px] text-[#171615]/70 leading-relaxed break-words">
                  {product.inci}
                </div>
              )}
            </div>
      ...
      {/* MOBILE STICKY BOTTOM BAR (Visible < lg screen sizes) */}
      <div className="fixed bottom-0 left-0 right-0 bg-[#F8F5F1]/95 backdrop-blur-md border-t border-[#C98F78]/30 p-3.5 z-40 lg:hidden shadow-[0_-8px_20px_rgba(23,22,21,0.06)]">
        <div className="max-w-md mx-auto flex items-center justify-between gap-3">
          <div className="flex flex-col">
            <span className="font-sans text-[10px] uppercase tracking-wider text-[#171615]/60 truncate max-w-[120px]">
              {product.name}
            </span>
            <span className="font-serif text-lg font-semibold text-[#171615]">{totalPrice}</span>
          </div>

          <button className="flex-1 max-w-[220px] bg-[#171615] text-[#F8F5F1] py-3 px-4 rounded-full font-sans text-[10px] uppercase tracking-[0.2em] font-semibold flex items-center justify-center gap-2 hover:bg-[#C98F78] active:scale-95 transition-all shadow-sm">
